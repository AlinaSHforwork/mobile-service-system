...
export default function Home() {
  ...
  return (
    <main style={{ minHeight: '100vh', background: 'var(--bg)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '2rem' }}>
      ...
      {/* Heading */}
      <h1 className="animate-fade-in stagger-1" style={{ fontSize: '2.5rem', fontWeight: 800, color: 'var(--text)', textAlign: 'center', marginBottom: '0.75rem' }}>
        Mobile Phone Repair Service
      </h1>
      <p className="animate-fade-in stagger-2" style={{ color: 'var(--text-muted)', textAlign: 'center', maxWidth: '480px', lineHeight: 1.6, marginBottom: '2.5rem' }}>
        Professional phone repair services with fast turnaround times. Track your repairs, get instant quotes, and stay updated on your device status.
      </p>

      {/* Feature cards */}
      <div className="animate-fade-in stagger-3" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '1rem', width: '100%', maxWidth: '900px', marginBottom: '2rem' }}>
        <FeatureCard
          icon={<WrenchIcon />}
      ...
      <div className="animate-fade-in stagger-4" style={{
        ...
        <h2 style={{ fontSize: '1.5rem', fontWeight: 700, marginBottom: '0.5rem', color: 'var(--text)' }}>Get Started</h2>
...
function FeatureCard({ icon, title, desc }) {
  return (
    <div style={{
      ...
      <div style={{ color: 'var(--primary)', marginBottom: '0.75rem', display: 'flex', justifyContent: 'center' }}>{icon}</div>
      <h3 style={{ fontWeight: 700, fontSize: '1rem', marginBottom: '0.5rem', color: 'var(--text)' }}>{title}</h3>
